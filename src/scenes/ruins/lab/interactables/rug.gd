...
func investigate():
	if (GameVars.get_var(Enums.Vars.MOVED_RUG)):
		Utility.msg("Ruins.Lab.Rug_Moved_Look")
	else:
		Utility.msg("Ruins.Lab.Rug_Look")

func interact():
	if (GameVars.get_var(Enums.Vars.MOVED_RUG)):
		Utility.msg("Ruins.Lab.Rug_Moved_Interact")
	else:
		SignalBus.teleport_player.emit(Vector2(417, 338), Enums.Direction.RIGHT)
		GameVars.set_var(Enums.Vars.MOVED_RUG, true)
		update_state()
		Utility.msg("Ruins.Lab.Rug_Interact")

func take():
	Utility.msg("Ruins.Lab.Rug_Take")
